...
// Handle PUT request to mark the task as completed.
export async function PUT(req, { params }) {
    try {
        const id = parseInt((await params).id, 10);
        const { processor, task_output } = await req.json();
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            // First let's try to renew the lease. This buys more time and prevents other processors from picking up the task if it expires.
            // Also it confirms we have a lease on the task.
            ...
            // Check if the lease was renewed successfully
            if (!leaseResponse.ok && leaseResponse.status !== 404) {
                ...
                const updatedTask = await tx.task.update({
                    data: {
                        ...
                        processedAt: lease.renewed_at, // dirty hack to use sql NOW(), is there another way?
        ...
        console.error('Error completing task: ', stringifyError(error));
